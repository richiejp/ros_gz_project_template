...
type ParameterDescriptor struct {
	Name string `yaml:"name" json:"name" mapstructure:"name"`// The name of the parameter.
	Type uint8 `yaml:"type" json:"type" mapstructure:"type"`// Enum values are defined in the `ParameterType.msg` message.
	Description string `yaml:"description" json:"description" mapstructure:"description"`// Description of the parameter, visible from introspection tools.
	AdditionalConstraints string `yaml:"additional_constraints" json:"additional_constraints" mapstructure:"additional_constraints"`// Plain English description of additional constraints which cannot be expressedwith the available constraints, e.g. "only prime numbers".By convention, this should only be used to clarify constraints which cannotbe completely expressed with the parameter constraints below.
	ReadOnly bool `yaml:"read_only" json:"read_only" mapstructure:"read_only"`// If 'true' then the value cannot change after it has been initialized.
	DynamicTyping bool `yaml:"dynamic_typing" json:"dynamic_typing" mapstructure:"dynamic_typing"`// If true, the parameter is allowed to change type.
	FloatingPointRange []FloatingPointRange `yaml:"floating_point_range" json:"floating_point_range" mapstructure:"floating_point_range"`// FloatingPointRange consists of a from_value, a to_value, and a step.
	IntegerRange []IntegerRange `yaml:"integer_range" json:"integer_range" mapstructure:"integer_range"`// IntegerRange consists of a from_value, a to_value, and a step.
}
